...
for i in range(M+1):
	...
	for k in range(len(testX)):
		length = clf.n_support_
		val = inter
		for l in range(length[0]):
			# val =val+ clf.dual_coef_[0][l]*compute_RBF(testX[k],clf.support_vectors_[l], (1/3)**(0.5)) 
			val =val+ clf.dual_coef_[0][l]*rbf_kernel([testX[k]],[clf.support_vectors_[l]]) 
